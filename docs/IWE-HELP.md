...
| **Артефакты** | Рабочие продукты | DS-my-strategy, Pack-репо, Цифровой двойник |
...
- DS-my-strategy — приватный репо для планирования
...
`DS-my-strategy/docs/WP-REGISTRY.md` — полный реестр всех РП от последнего к первому (DP.WP.015). Обновляется на Close при изменении статуса.
...
НЕ трогаются: MEMORY.md, DS-my-strategy/, routing.md, личные настройки.
...
Заметки попадают в `DS-my-strategy/inbox/fleeting-notes.md`. Стратег разбирает вечером (Note-Review).
...
**DS-my-strategy не создан** — вручную: `mkdir -p ~/Github/DS-my-strategy/{current,inbox,docs,archive} && cd ~/Github/DS-my-strategy && git init`.
...
| DS-my-strategy | Личный стратегический хаб (приватный репо) |
