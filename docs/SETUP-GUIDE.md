...
- **DS-my-strategy** — твой личный стратегический хаб (приватный репозиторий на GitHub)
...
Создай на своём компьютере **одну папку** для всех репозиториев — текущих и будущих. В неё будут клонироваться все репозитории: `FMT-exocortex-template/`, `DS-my-strategy/`, `PACK-{область}/`, `DS-{проекты}/` и др. `CLAUDE.md` тоже будет лежать в корне этой папки. По умолчанию это `~/Github`:
...
6. Создаст `DS-my-strategy/` — твой приватный стратегический репозиторий на GitHub
...
**DS-my-strategy не создан:**
```bash
cd ~/Github
mkdir -p DS-my-strategy/{current,inbox,docs,archive/wp-contexts,exocortex}
cd DS-my-strategy && git init && git add -A && git commit -m "Initial"
gh repo create $(gh api user -q .login)/DS-my-strategy --private --source=. --push
```
</details>
...
**Результат:** заполненные `DS-my-strategy/docs/Strategy.md`, `Dissatisfactions.md` и первый `WeekPlan` в `DS-my-strategy/current/`.
...
Заметка попадёт в `DS-my-strategy/inbox/fleeting-notes.md`. Стратег разберёт её вечером (Note-Review, 23:00) и классифицирует: задача → план, знание → captures, идея → на обсуждение.
...
|-------|-------|---------------|---------------|
| **Утро (Вт-Вс)** | Стратег | Собирает коммиты за вчера, формирует план дня | `DS-my-strategy/current/DayPlan YYYY-MM-DD.md` |
| **Утро (Пн)** | Стратег | Готовит черновик недельного плана + повестку сессии | `DS-my-strategy/current/WeekPlan W{N}.md` |
| **Каждые 3 часа** | Экстрактор* | Проверяет inbox (заметки, captures) → предлагает знания в Pack | `DS-my-strategy/inbox/extraction-reports/` |
| **Вечер (23:00)** | Стратег | Note-Review классифицирует заметки из Telegram | Целевые документы в DS-my-strategy |
| **Ночь (00:00)** | Синхронизатор* | Code-scan — обзор изменений в downstream-репо | `DS-my-strategy/current/CodeScan YYYY-MM-DD.md` |
| **Ночь (Вс→Пн)** | Стратег | Week Review — итоги недели | `DS-my-strategy/current/WeekReport W{N}.md` |
| **Утро (06:00)** | Синхронизатор* | Daily report — сводка ночных задач | `DS-my-strategy/current/SchedulerReport YYYY-MM-DD.md` |
...
Что НЕ трогается: MEMORY.md, DS-my-strategy/, routing.md, пользовательские permissions — всё пользовательское (user-space).
...
| DS-my-strategy | Приватный репо на GitHub | Только на GitHub (private) |
...
DS-my-strategy — приватный репо. MEMORY.md — локальный файл. Ничего не публикуется без твоего ведома. Подробности о том, что отправляется на внешние серверы (Claude API, WakaTime, GitHub) — см. раздел [Безопасность и приватность](#безопасность-и-приватность).
