...
2. Заменяет 7 плейсхолдеров (`testo97`, `/Users/user/Github` и др.)
...
2. Замените `/Users/user/Github` на путь к рабочей директории (напр. `~/Github`)
3. Замените `/Users/user` на домашнюю директорию (значение `$HOME`)
4. Замените `-Users-user-Github` на путь через дефисы (напр. для `~/Github` → `-Users-yourname-Github`)
5. Замените `2` на час запуска стратега в UTC (напр. `4` для 7:00 MSK)
6. Замените `7:00 EKB` на описание времени (напр. `7:00 MSK`)
