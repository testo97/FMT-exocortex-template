...
2. Заменяет 7 плейсхолдеров (`testo97`, `/home/misha/Github` и др.)
...
1. Замените `testo97` на ваш GitHub username во всех файлах
2. Замените `/home/misha/Github` на путь к рабочей директории (напр. `~/Github`)
3. Замените `/home/misha` на домашнюю директорию (значение `$HOME`)
4. Замените `-home-misha-Github` на путь через дефисы (напр. для `~/Github` → `-Users-yourname-Github`)
5. Замените `2` на час запуска стратега в UTC (напр. `4` для 7:00 MSK)
6. Замените `5:00` на описание времени (напр. `7:00 MSK`)
7. Замените `/home/misha/.local/bin/claude` на путь к Claude CLI (напр. `/opt/homebrew/bin/claude`)
